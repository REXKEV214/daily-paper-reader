...
tldr: 针对大语言模型在语义过滤任务中逐行扫描导致的高延迟和高成本问题，本文提出了一种名为CSV（聚类-采样-投票）的新框架。该框架通过将元组嵌入语义聚类并仅对少量样本进行LLM评估，利用UniVote和SimVote投票策略推断聚类标签，实现了亚线性的调用复杂度。实验表明，CSV在保持高准确率的同时，显著降低了LLM调用次数，为大规模语义查询提供了高效且低成本的解决方案。
selection_source: fresh_fetch
motivation: 传统的语义过滤需要对表格进行逐行线性扫描，导致在大规模语料库上处理时面临极高的延迟和Token成本。
method: 提出CSV框架，通过语义聚类、小样本采样及UniVote/SimVote投票机制来推断标签，并引入重聚类机制处理模糊簇。
result: 在真实数据集上，CSV相比现有最先进方法减少了1.28至355倍的LLM调用次数，且保持了相当的准确率和F1分数。
conclusion: 该研究证明了通过聚类和采样策略可以将LLM语义过滤的复杂度降至亚线性，为高效的大规模语义查询处理提供了新范式。
---

## 摘要
大语言模型（LLMs）越来越多地被用于大规模语料库上的语义查询处理。一系列源自关系代数的语义算子被提出，旨在为表达此类查询提供统一接口，其中语义过滤算子是其核心基石。给定一个带有自然语言谓词 e 的表 T，对于关系中的每个元组，语义过滤的执行过程包括：构建一个结合了谓词 e 及其内容的输入提示词（prompt），查询 LLM，并获取二元决策结果。然而，这种逐元组的评估方式需要对表进行完整的线性扫描，从而导致极高的延迟和 Token 成本。尽管最近的研究尝试优化语义过滤，但仍未能打破线性 LLM 调用的障碍。为解决这一问题，我们提出了“聚类-采样-投票”（Clustering-Sampling-Voting, CSV）框架，该框架在提供误差保证的同时，将 LLM 调用次数降低至亚线性复杂度。CSV 将元组嵌入到语义簇中，采样一小部分子集进行 LLM 评估，并通过两种提出的投票策略推断簇级标签：UniVote（均匀聚合标签）和 SimVote（根据语义相似度对投票进行加权）。此外，CSV 会对模糊簇触发重新聚类，以确保在不同数据集上的鲁棒性。在真实数据集上进行的实验结果表明，与现有最先进的方法相比，CSV 将 LLM 调用次数减少了 1.28 到 355 倍，同时在准确率（Accuracy）和 F1 分数方面保持了相当的有效性。
