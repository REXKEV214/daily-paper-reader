...
date: 2026-03-03
pdf: "https://arxiv.org/pdf/2603.02678v2"
tags: ["query:hci-ai"]
score: 7.0
evidence: 人机回环与交互式知识获取
tldr: 针对从观测数据中学习因果结构（DAG）面临的组合爆炸和歧义性挑战，本文提出了一种融合“群体智慧”的新范式。该范式通过众包平台、专家知识建模、鲁棒聚合技术以及大语言模型（LLM）模拟，将因果发现转化为分布式决策任务。通过整合人类专家和AI代理的碎片化知识，该框架旨在恢复单个个体无法实现的全局因果结构，为因果学习开辟了结合人类先验与机器智能的新研究前沿。
selection_source: fresh_fetch
motivation: 传统的因果结构学习受限于搜索空间巨大和观测数据不足，难以准确恢复复杂的因果关系。
method: 提出一个系统性框架，整合众包数据采集、交互式知识提取、鲁棒专家意见聚合以及基于LLM的信息增强。
result: 将因果发现建模为分布式决策任务，通过合成多个代理的局部碎片化知识来构建全局因果图。
conclusion: 倡导因果学习应拥抱群体智慧，并为未来从知识提取到优化的研究方向提供了全面的路线图。
---

## 摘要
从观测数据中学习通常由有向无环图（DAG）表示的因果结构，由于可能图结构的组合爆炸以及观测数据中固有的歧义性，一直是一项极具挑战性的任务。本文认为，在快速发展的技术支持下，因果学习现在已经准备好迎接一种新范式的出现，从而实现利用人类因果知识的长期愿景。该范式整合了用于数据收集的可扩展众包平台、用于专家意见建模的交互式知识引导、用于专家意见协调的鲁棒聚合技术，以及用于增强 AI 驱动信息获取的基于大语言模型（LLM）的模拟。在本文中，我们专注于因果发现中的 DAG 学习，并将该问题框架化为一个分布式决策任务，承认每个参与者（人类专家或 LLM 智能体）对因果图中感兴趣变量的不同子集仅拥有碎片化且不完善的知识。通过提出一个系统性的框架来合成这些见解，我们的目标是实现全局因果结构的恢复，而这是任何单个智能体都无法独立完成的。我们倡导一个新的研究前沿，并勾勒出一个涵盖从引导、建模、聚合到优化人类因果知识贡献的新研究方向的综合框架。
