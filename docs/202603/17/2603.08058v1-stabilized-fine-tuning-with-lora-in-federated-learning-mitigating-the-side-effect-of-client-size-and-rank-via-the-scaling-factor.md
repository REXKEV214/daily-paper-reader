...
method: 提出 SFed-LoRA 框架，通过理论推导得出一种结合客户端数量与秩的优化缩放因子，以修正聚合过程中的缩放偏差。
result: 实验表明 SFed-LoRA 有效防止了高秩崩溃，在多种任务和异构数据分布下均实现了比现有基准更优的稳定性和收敛表现。
conclusion: 该研究通过优化缩放因子解决了联邦学习中 LoRA 的不稳定性，为在分布式环境下高效利用高秩适配器提供了理论支持和实践方案。
---
