...
tags: ["query:llm-bias"]
score: 8.0
evidence: 鲁棒的 RLHF 以防止在适配过程中遗忘安全行为
tldr: 针对大语言模型在多阶段训练中容易出现灾难性遗忘（如安全性下降）的问题，本文提出了FRPO框架。不同于以往在下游微调阶段进行干预的方法，FRPO在RLHF阶段通过最大最小化公式，优化当前策略及其KL邻域内所有潜在策略的奖励稳定性。该方法基于GRPO实现，无需额外计算开销，在保持下游任务性能的同时，显著提升了模型对后续微调的鲁棒性。
selection_source: carryover_cache
motivation: 现有的RLHF目标缺乏对未来适配的鲁棒性，导致微小的下游微调即可破坏模型已习得的关键行为（如安全性）。
method: 提出FRPO框架，通过在KL散度约束的邻域内优化奖励稳定性，确保模型在策略偏移下仍能保持高奖励。
result: 在多个基础模型和微调场景（SFT和RL）下，FRPO显著减少了安全性退化，并保持了下游任务的准确性。
conclusion: 通过在预微调阶段引入鲁棒性优化，可以有效缓解大模型的灾难性遗忘问题，提升模型在持续学习中的稳定性。
---
