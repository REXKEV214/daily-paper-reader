...
tags: ["query:hci-ai"]
score: 8.0
evidence: AI图像细化的用户体验设计
tldr: 本研究探讨了如何利用手写草图和标注来增强生成式AI图像的精细化调整。针对现有文本提示在处理布局、比例等空间细节时的局限性，研究者开发了一个支持纯文本、纯视觉及组合输入的原型系统。通过对30名设计师的对比实验发现，视觉提示在空间编辑上更高效，而组合模式在表达力、用户体验和迭代流畅度上表现最佳，为未来GenAI设计工具的界面优化提供了重要参考。
selection_source: carryover_cache
motivation: 现有的生成式AI工具在图像精细化调整（如布局和比例）方面缺乏精确性，且纯文本提示在处理空间关系时效率低下。
method: 开发了一个集成草图和标注的多模态原型，并邀请30名专业人士和学生对比研究了纯文本、纯视觉及组合提示在不同任务下的表现。
result: 视觉提示显著提升了空间编辑的清晰度与速度并降低了工作量，而组合模式因其互补性在整体评价和迭代支持上得分最高。
conclusion: 多模态输入是提升GenAI图像精细化效率的关键，未来设计工具应结合视觉精准度与文本语义能力以支持更复杂的创作流。
---
